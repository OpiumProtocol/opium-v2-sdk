...
import { Contract, CallOverrides, ContractReceipt } from '@ethersproject/contracts';
...
export class CoreContract {
  ...
  public async create(
    ...
    const tokenSpenderAddress = await this._coreService.contract.getProtocolAddresses();
    const token = <IERC20>new Contract(_derivative.token, IERC20Abi, this._coreService.getProvider());

    const requiredMargin = await this._computeDerivativeMargin(_derivative, _amount);
    await token.connect(signer).approve(tokenSpenderAddress.tokenSpender, requiredMargin);
    const tx = await this._coreService.contract.connect(signer).create(_derivative, _amount, _positionsOwners);
    return tx.wait();
  }
  ...
  public async executeOneWithAddress(
    ...
    const tx = await this._coreService.contract['execute(address,address,uint256)'](
      _positionOwner,
      _positionAddress,
      _amount,
      _overrides,
  ...
  public async getReservesVaultBalance(_reseveRecipient: string, _tokenAddress: string): Promise<BigNumberish> {
    return this._coreService.contract.getReservesVaultBalance(_reseveRecipient, _tokenAddress);
  }

  public async getProtocolAddresses(): Promise<TProtocolAddresses> {
    return struct(omit(await this._coreService.contract.getProtocolAddresses(), '__gapOne', '__gapTwo'));
  }
  ...
  private async _getDerivativePayouts(_derivativeHash: string): Promise<[BigNumberish, BigNumberish]> {
    return this._coreService.contract.getDerivativePayouts(_derivativeHash);
  }

  private async _isDerivativeCancelled(_derivativeHash: string): Promise<boolean> {
    return this._coreService.contract.isDerivativeCancelled(_derivativeHash);
  }

  private async _computeDerivativeMargin(_derivative: TDerivative, _amount: BigNumberish): Promise<BigNumberish> {
    const syntheticId = <IDerivativeLogic>(
      new Contract(_derivative.syntheticId, IDerivativeLogicAbi, this._coreService.getProvider())
    );
