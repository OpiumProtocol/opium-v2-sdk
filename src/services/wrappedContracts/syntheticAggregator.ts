...
export class SyntheticAggregatorContract {
  ...
  public async getOrCacheMargin(_derivative: TDerivative, _overrides: CallOverrides = {}) {
    const signer = (await this._syntheticAggregatorService.getProvider()).getSigner();
    const derivativeHash = getDerivativeHash(_derivative);
    return this._syntheticAggregatorService.contract.connect(signer).getOrCacheMargin(derivativeHash, _derivative);
  }
