...
export class OracleAggregatorContract {
  ...
  public async pushData(
    ...
    const signer = (await this._oracleAggregatorService.getProvider()).getSigner();
    return this._oracleAggregatorService.contract.connect(signer).__callback(_timestamp, _data, _overrides);
  }

  public async getData(_oracleId: string, _timestamp: BigNumber, _overrides: CallOverrides = {}): Promise<BigNumber> {
    return this._oracleAggregatorService.contract.getData(_oracleId, _timestamp, _overrides);
  }

  public async hasData(_oracleId: string, _timestamp: BigNumber, _overrides: CallOverrides = {}): Promise<boolean> {
    return this._oracleAggregatorService.contract.hasData(_oracleId, _timestamp, _overrides);
  }
}
